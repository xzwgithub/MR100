...
//    NSLog(@"fsubsession 的 mediumName : %s",fSubsession.mediumName()); //崩溃过
    //    NSString *str = [[NSString alloc] initWithBytes:fReceiveBuffer length:100000 encoding:NSUTF8StringEncoding];
    //    NSLog(@"%@",str);
    

    //丢掉延迟帧(迟到的帧)
//    NSDate *nowDate = [NSDate date];
//    int now_UTC = [nowDate timeIntervalSince1970];
//    if (abs(now_UTC-presentationTime.tv_sec-presentationTime.tv_usec)>0.5)
//    {
//        continuePlaying();
//        return;
//    }
...
//    if (!strcmp(fSubsession.codecName(), "JPEG"))
//    {
//        [[QueueBuffer shareStore] addBuffer:fReceiveBuffer Framesize:frameSize Presentation:presentationTime];
//    }
    ...
    //若str1<str2，则返回负数；
    //若str1>str2，则返回正数。
    envir() << "\t NUM: " << fSubsession.rtpInfo.seqNum;
    ...
    else if (!strcmp(fSubsession.mediumName(), "video"))
    {
        ...
        if(firstFrame)
        {
            ...
            }else if (!strcmp(fSubsession.codecName(), "H264"))
            {
                unsigned int num;
                ...
                if(rtspConnection.delegate)
                {
                    [rtspConnection.delegate gotSps:sps[0].sPropBytes SpsSize:sps[0].sPropLength Pps:sps[1].sPropBytes PpsSize:sps[1].sPropLength];
                }
...
//    [Decoder_Ffmpeg ffmpeg_264_decoder].delegate = delegate;
    ...
    NSString *str=[NSString stringWithFormat:online?online_play:h264_add];  //h264
